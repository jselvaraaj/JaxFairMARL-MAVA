...
class JaxMarlFairSpread(SimpleSpreadMPE):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        new_obs_size = self.observation_space(self.agents[0]).shape[0] + 2
    ...
    def get_obs(self, state: FairSpreadState) -> Dict[str, chex.Array]:
        ...
        relative_pos = _get_assigned_landmark_info(self.agent_range)

        obs = {
            agent: jnp.concatenate([base_obs[agent], relative_pos[i]])
